...
-(void)configureSubViews
{
    ...
    [self.indicatorView addSubview:self.activity];
}
...
-(void)showAtView:(UIView *)view hudType:(JHUDLoadingType)hudType
{
    ...
    [self dispatchMainQueue:^{
        ...
        view ? [view addSubview:self_weak_]:[KLastWindow addSubview:self_weak_];
        
        [self_weak_ mas_updateConstraints:^(MASConstraintMaker *make) {
            make.edges.mas_equalTo(0);
        }];
        
        [self_weak_.superview bringSubviewToFront:self_weak_];
    }];
}
...
-(void)hide
{
    @weakify(self);
    [self dispatchMainQueue:^{
        if (self_weak_.superview) {
            [self_weak_ removeFromSuperview];
            [self_weak_.loadingView removeSubLayer];
        }
    }];
}
...
- (void)setupSubViewsWithHudType:(JHUDLoadingType)hudType
{
    ...
    switch (hudType) {
        ...
        case JHUDLoadingTypeFailure:
            break;
        case JHUDLoadingTypeActivityIndicator:
...
- (UIActivityIndicatorView *)activity{
    ...
    if (!_activity) {
        ...
        _activity.tintColor = UIColor.groupTableViewBackgroundColor;
    }
    return _activity;
...
-(UILabel *)messageLabel
{
    ...
    self.messageLabel.font = m_FontPF_Regular_WithSize(14);
...
-(UIButton *)refreshButton
{
    ...
    self.refreshButton.titleLabel.font = m_FontPF_Regular_WithSize(14);
...
-(void)updateConstraints
{
    ...
    [self.activity removeAllConstraints];
    ...
    [self addConstarintWithTopView:self.indicatorView toBottomView:self.messageLabel constarint:self.indicator?-(self.indicatorViewSize.height/4):40];
    ...
    [self.refreshButton addConstraintWidth:130 height:40];
