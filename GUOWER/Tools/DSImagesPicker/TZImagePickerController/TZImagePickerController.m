...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = GW_OrangeColor;
...
- (id)showAlertWithTitle:(NSString *)title {
    ...
    MMAlertView *alert = [[MMAlertView alloc]initWithTitle:title detail:nil items:@[MMItemMake([NSBundle tz_localizedStringForKey:@"OK"], MMItemTypeNormal, nil)]];
    [alert show];
    return alert;
...
//    alertView = nil;
    ...
    if ([alertView isKindOfClass:MMAlertView.class]) {
        MMAlertView *alert = alertView;
        [alert hide];
    }
    alertView = nil;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    RTContainerController *rtcon = [self.childViewControllers firstObject];
    
    TZAlbumPickerController *albumPickerVc = rtcon.contentViewController;
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:imagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:imagePickerVc action:@selector(cancelButtonClick)];
