...
#define kHostsCornerRadius 4.0f
...
+ (instancetype)cancelSection {
    return [self sectionWithTitle:nil message:nil buttonTitles:@[NSLocalizedString(@"Cancel",)] buttonIcons:nil buttonStyle:JGActionSheetButtonStyleCancel];
}

+ (instancetype)sectionWithTitle:(NSString *)title message:(NSString *)message buttonTitles:(NSArray *)buttonTitles buttonIcons:(NSArray *)buttonIcons buttonStyle:(JGActionSheetButtonStyle)buttonStyle {
    return [[self alloc] initWithTitle:title message:message buttonTitles:buttonTitles buttonIcons:buttonIcons buttonStyle:buttonStyle];
}

- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message buttonTitles:(NSArray *)buttonTitles buttonIcons:(NSArray *)buttonIcons buttonStyle:(JGActionSheetButtonStyle)buttonStyle {
    self = [super init];
    ...
    if (self) {
        if (title) {
            ...
            titleLabel.font = m_FontPF_Medium_WithSize(14);
        ...
        if (message) {
            ...
            messageLabel.font = m_FontPF_Regular_WithSize(12);
        ...
        if (buttonTitles.count) {
            ...
            for (NSString *str in buttonTitles) {
                ...
                if (buttonIcons) {
                    icon = [buttonIcons objectAtIndex:[buttonTitles indexOfObject:str]];
                }
                
                JGButton *b = [self makeButtonWithTitle:str icon:icon style:buttonStyle];
                ...
                if (index<buttonTitles.count-1) {
                    ...
                    [line mas_makeConstraints:^(MASConstraintMaker *make) {
                        ...
                        make.top.equalTo(b.mas_bottom).mas_offset(2);
...
- (void)setButtonStyle:(JGActionSheetButtonStyle)buttonStyle forButton:(UIButton *)button {
    ...
    if (buttonStyle == JGActionSheetButtonStyleDefault) {
        font = m_FontPF_Regular_WithSize(14);
...
- (JGButton *)makeButtonWithTitle:(NSString *)title icon:(NSString *)icon style:(JGActionSheetButtonStyle)style {
    ...
    if (image) {
        ...
        [b layoutButtonWithEdgeInsetsStyle:MKButtonEdgeInsetsStyleLeft imageTitleSpace:10];
    }
    ...
    if (_targetView && !CGRectEqualToRect(self.bounds, _targetView.bounds)) {
        ...
        [UIView animateWithDuration:(iPad ? 0.4 : 0.3) delay:0.0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
            if (self->_anchoredAtPoint) {
                [self_weak_ moveToPoint:self->_anchorPoint arrowDirection:self->_anchoredArrowDirection animated:NO];
            }
            else {
...
- (void)moveToPoint:(CGPoint)point arrowDirection:(JGActionSheetArrowDirection)arrowDirection animated:(BOOL)animated {
    ...
    @weakify(self);
    void (^changes)(void) = ^{
        self.frame = self_weak_.targetView.bounds;
        ...
        if (arrowDirection == JGActionSheetArrowDirectionRight) {
            ...
            finalFrame.size.height = CGRectGetHeight(self_weak_.targetView.bounds);
        }
        else if (arrowDirection == JGActionSheetArrowDirectionLeft) {
            finalFrame.size.width = CGRectGetWidth(self_weak_.targetView.bounds)-point.x-arrowHeight;
            finalFrame.size.height = CGRectGetHeight(self_weak_.targetView.bounds);
            finalFrame.origin.x = point.x+arrowHeight;
        }
        else if (arrowDirection == JGActionSheetArrowDirectionTop) {
            finalFrame.size.width = CGRectGetWidth(self_weak_.targetView.bounds);
            finalFrame.size.height = CGRectGetHeight(self_weak_.targetView.bounds)-point.y-arrowHeight;
        ...
        finalFrame = UIEdgeInsetsInsetRect(finalFrame, self_weak_.insets);
        ...
        [self_weak_ layoutSheetForFrame:finalFrame fitToRect:NO initialSetUp:YES continuous:YES];
        
        [self_weak_ anchorSheetAtPoint:point withArrowDirection:arrowDirection availableFrame:finalFrame];
    };
